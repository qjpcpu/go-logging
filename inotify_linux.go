...
func (f *filter) passby(ev *Event) bool {
    t := time.Now()
    key := fmt.Sprintf("%s:%v", ev.Name, ev.Mask)
    if _, ok := f.memo[key]; !ok {
        f.memo[key] = t
        return true
    } else {
        if f.memo[key].Add(f.interval).Before(t) {
            f.memo[key] = t
            return true
        } else {
            return false
